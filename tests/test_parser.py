...
class ParserTestCase(unittest.TestCase):
    ...
    def test_24fps(self):
        ...
        self.assertEqual(
            s.events[2].clip_name, "clip -3", "Failed clip name test char 3"
        )
        self.assertEqual(
            s.events[3].clip_name, "clip $4", "Failed clip name test char 4"
        )
        self.assertEqual(
            s.events[4].clip_name, "clip &5", "Failed clip name test char 5"
        )
        self.assertEqual(
            s.events[5].src_start_tc.frame_number,
            697,
